...
interface MatchResult {
  ...
  match: (TariffItem & { score?: number; keywordScore?: number; stringScore?: number; categories?: string[] }) | null;
...
const App = () => {
  ...
  const findBestMatch = (inventoryItem: InventoryItem): (TariffItem & { score: number; keywordScore: number; stringScore: number; categories: string[] }) | null => {
  ...
  const processCsvData = (csvText: string): Record<string, string>[] => {
  ...
  const handleFileUpload = (event: React.ChangeEvent<HTMLInputElement>, type: 'tariff' | 'inventory'): void => {
    ...
    reader.onload = (e) => {
      try {
        ...
        if (type === 'tariff') {
          ...
          setTariffData(data as TariffItem[]);
        } else {
        ...
        alert(`Error parsing ${type} file: ${err.message}`);
        console.error(error);
      }
    };
